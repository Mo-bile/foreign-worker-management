...
class IndustrialAccidentPolicyTest {

    private final IndustrialAccidentPolicy policy = new IndustrialAccidentPolicy();

    @Test
    @DisplayName("모든 외국인 근로자는 산재보험 의무가입")
    void allWorkersShouldBeMandatory() {
        ForeignWorker worker = createWorker(Nationality.CAMBODIA, VisaType.E9);
        ...
        assertThat(result.insuranceType()).isEqualTo(InsuranceType.INDUSTRIAL_ACCIDENT);
    }

    @Test
    @DisplayName("비자 유형과 무관하게 산재보험 의무가입")
    void differentVisaTypesShouldAllBeMandatory() {
        for (VisaType visaType : VisaType.values()) {
            ForeignWorker worker = createWorker(Nationality.INDONESIA, visaType);
    ...
    @Test
    @DisplayName("산재보험 판정 사유 메시지 포함 확인")
    void shouldIncludeCorrectReasonMessage() {
        ForeignWorker worker = createWorker(Nationality.NEPAL, VisaType.H2);

        InsuranceEligibility result = policy.determineEligibility(worker);

        assertThat(result.reason()).contains("산재보험");
    }
