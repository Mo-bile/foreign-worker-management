...
2. **임금 경쟁력**: `public_data_keis_worker`의 동일 지역·업종·규모 외국인 근로자 임금 분포 → 백분위 산출
3. **인력 유출 위험**: `public_data_keis_worker`의 재고용여부·이동이력·근무상태 분석
...
| API | 제공기관 | 용도 | 단계 |
|-----|----------|------|------|
| 고용/산재보험 현황정보 | 근로복지공단 (후원기관) | 사업장 보험가입 검증 + 자동채움 | 1~2단계 |
| 국가법령정보센터 | 법제처 | 법령 개정 모니터링 | 3단계 |

**근로복지공단 API 상세:**
- data.go.kr 활용신청 (자동승인)
- 요청: 사업자등록번호(`v_saeopjaDrno`), 산재/고용 구분(`opaBoheomFg`)
- 응답: 사업장명, 주소, 상시인원, 성립일자, 고용업종코드/명, 산재업종코드/명, 보험가입구분
- 활용: 사업장 등록 시 사업자등록번호 입력 → API 조회 → 사업장명·주소·상시인원·업종 자동채움 + 보험가입 상태 검증
...
**표준 (+ 2단계):**
- 위 + 벤치마크 API + 15105236 적재 + 근로복지공단 API 연동 + 데드라인 관리

**전체 (+ 3단계):**
- 위 + 법령 변경 감지 배치(법제처 API) + 영향 분석 + 컴플라이언스 리포트
